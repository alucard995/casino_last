...
class Prizes{
	...
	public static function addUserPrize($userId, $prizeType = '1', $prizeId = '', $prizeValue){
		if($prizeType == 1){
	...
	public static function deleteUserPrize($userId, $prizeId){
		$prizeInfo = [];
    	...
    	if($prizeInfo['prize_id'] != ''){
			if($prizeInfo['prize_type_id'] == 2){
				$deleteUserPrize = db\database::startConnect('DELETE FROM `user_prizes` WHERE `user_id` = :userId && `prize_id` = :prizeId', 
					array('userId' => $userId, 'prizeId' => $prizeInfo['prize_id']));
				$deleteBonuse = db\database::startConnect('DELETE FROM `money` WHERE `money_id` = :prizeId', array('prizeId' => $prizeInfo['prize_id']));
			}
			elseif($prizeInfo['prize_type_id'] == 3){
				$deleteUserPrize = db\database::startConnect('DELETE FROM `user_prizes` WHERE `user_id` = :userId && `prize_id` = :prizeId', 
					array('userId' => $userId, 'prizeId' => $prizeInfo['prize_id']));
				$deleteBonuse = db\database::startConnect('DELETE FROM `bonuses` WHERE `bonuse_id` = :prizeId', array('prizeId' => $prizeInfo['prize_id']));
			}
			else{
				$deleteUserPrize = db\database::startConnect('DELETE FROM `user_prizes` WHERE `user_id` = :userId && `prize_id` = :prizeId', 
					array('userId' => $userId, 'prizeId' => $prizeInfo['prize_id']));
